...
def download_patch_file():
    if maxbit > 2**32:  # 64位
        ...
        url = (
            f"https://github.com/LiteLoaderQQNT/QQNTFileVerifyPatch/releases/download/"
            + tag_data["tag_name"]
            + "/dbghelp_x64.dll"
        )
            ...
            def compare_two_size():  # 比较俩文件是不是一样的
                ...
                for asset in tag_data.get("assets", []):
                    if (
                        ...
                        == f"https://github.com/LiteLoaderQQNT/QQNTFileVerifyPatch/releases/download/{tag_NAME}/dbghelp_x64.dll"
                    ):  # 2024/8/13 20:43 这里不知道哪个智障给我生成了个错误的URL导致我排查了好久，望周知。
        ...
        if total_size != 0 and wrote != total_size:
            ...
            def compare_two_size():  # 比较俩文件是不是一样的
                ...
                for asset in tag_data.get("assets", []):
                    if (
                        asset.get("browser_download_url")
                        == f"https://github.com/LiteLoaderQQNT/QQNTFileVerifyPatch/releases/download/{tag_NAME}/dbghelp_x86.dll"
                    ):  # 在上面的x64分支：2024/8/13 20:43 这里不知道哪个智障给我生成了个错误的URL导致我排查了好久，望周知。
                        sizes = asset.get("size")
                if local_size == sizes:  # 如果文件大小一致
                    logging.info("补丁文件大小正确，通过检测")
                    print("补丁文件大小校验完毕，正确，通过检测")
...
if os.path.exists(f"{localappdata}/LiteLoaderQQNT Auto Patch/Install_cancel_skip.ini"):
    with open(
        f"{localappdata}/LiteLoaderQQNT Auto Patch/Install_cancel_skip.ini",
        "r",
        encoding="utf8",
    ) as check_install_cancel_skip:
...
if (
    skip_install_dbghelp == "False"
):  # 如果这个永久选择是False的话就执行，如果是True的话就跳过，并直接打开QQ
    if not os.path.exists(f"{vape}/dbghelp.dll"):
        ...
        if install_cancel_skip == "1":
            ...
            if api.status_code == 200:
                ...
                print(
                    "当前补丁的tag_name为：" + tag_data["tag_name"]
                )  # 获取当前的版本号 tag_name
